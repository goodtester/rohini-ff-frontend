...
export class UserService {
  ...
  createUser(user: User) {
    ...
    return this.http.post<UserInterface>(`${this._urlBackendApi}/user/`, payload, {
      headers,
    });
  }
  ...
  editUsers(user: User, idUser: number) {
    const headers = new HttpHeaders().set('Authorization', `Bearer ${localStorage.getItem('token')}` || '');
    const payload = new HttpParams().set('request', JSON.stringify(user));
    return this.http.put<UserInterface>(`${this._urlBackendApi}/user/${idUser}`, payload, {
      headers,
    });
  }
  ...
  deleteUser(idUser: number) {
    const headers = new HttpHeaders().set('Authorization', `Bearer ${localStorage.getItem('token')}` || '');
    return this.http.delete<UserInterface>(`${this._urlBackendApi}/user/${idUser}`, {
      headers,
    });
  }
}
