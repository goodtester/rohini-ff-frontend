...
export class ReportService {
  ...
  getRankProducts(idProduct: string | null, limit: string | null, startDate: Date | null, endDate: Date | null) {
    ...
    return this.http.get<ReportProduct>(`${this._urlBackendApi}/report/product/?${valueParams}`, { headers });
  }

  getClient(username: string | null, startDate: Date | null, endDate: Date | null) {
    const headers = new HttpHeaders().set('Authorization', `Bearer ${localStorage.getItem('token')}` || '');
    let valueParams = '';
    if (username != null) {
    ...
    valueParams += this.validateDate(startDate, endDate);
    return this.http.get<ReportClient>(`${this._urlBackendApi}/report/client/?${valueParams}`, { headers });
  }
  ...
  getSalesMonthly() {
    const headers = new HttpHeaders().set('Authorization', `Bearer ${localStorage.getItem('token')}` || '');
    return this.http.get<ReportSalesMontly>(`${this._urlBackendApi}/report/salesmonthly/`, { headers });
  }

  getSalesWeekly() {
    const headers = new HttpHeaders().set('Authorization', `Bearer ${localStorage.getItem('token')}` || '');
    return this.http.get<ReportSalesWeekly>(`${this._urlBackendApi}/report/salesweekly/`, { headers });
  }

  getQuatityPayMode() {
    const headers = new HttpHeaders().set('Authorization', `Bearer ${localStorage.getItem('token')}` || '');

    return this.http.get<ReportPayMode>(`${this._urlBackendApi}/report/paymode/`, { headers });
  }
}
