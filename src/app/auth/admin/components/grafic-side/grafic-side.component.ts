...
export class GraficSideComponent implements OnInit {
  constructor(private reportService: ReportService, private graficPage: GraficsComponent) {}
  ...
  getClient(params: string, e: any) {
    ...
    this.reportService.getClient(this.username, this.startDateClient, this.endDateClient).subscribe((listUser) => {
  ...
  getProduct(params: string, e: any) {
    ...
    this.reportService
      .getRankProducts(this.idProduct, this.limit, this.startDateProduct, this.endDateProduct)
