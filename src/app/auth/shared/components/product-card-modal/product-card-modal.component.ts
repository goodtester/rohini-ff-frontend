...
export class ProductCardModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.product.category.idCategory != null) {
      this.homeService.getIngredientes(this.product.category.idCategory).subscribe((Additionals) => {
  ...
  addAdditional(Additional: Additional) {
    ...
    this.AdditionalSelected.sort((a, b) => (a.name < b.name ? -1 : a.name > b.name ? 1 : 0));
  }
