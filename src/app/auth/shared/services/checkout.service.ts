...
export class CheckoutService {
  ...
  constructor(private http: HttpClient, private orderService: OrderService, private route: Router) {}
  ...
  createBill(newBill: CreateBillParams, method: number) {
    ...
    this.http.post<CreateBillResponse>(`${this._urlBackendApi}/bill`, newBill).subscribe((response) => {
  ...
  createOrder(newOrder: AddCartInterface[], method: number) {
    newOrder.forEach((element) => {
      this.http.post<OrderResponse>(`${this._urlBackendApi}/orders`, element).subscribe(() => {
        this.orderService.deleteOrder();
        if (method == 3) {
          let element: HTMLElement = document.getElementsByClassName('pagos')[0] as HTMLElement;
          element.click();
        } else {
  ...
  setTokenBill(idBill: number) {
    this.http.get<TokenBill>(`${this._urlBackendApi}/token-refresh/unattributed/${idBill}`).subscribe((token) => {
      localStorage.setItem('bill', token.data?.unattributed.toString()!);
    });
  }

  getBill() {
    const headers = new HttpHeaders().set('Authorization', `Bearer ${localStorage.getItem('bill')}` || '');
    return this.http.get<BillInformation>(`${this._urlBackendApi}/bill/unattributed/`, {
      headers,
    });
  }
